...
export default function FacultyAttendanceReportsPage() {
  ...
  const dayWiseData = useMemo(() => {
    // Filter records by faculty's subjects
    const facultySubjectIds = facultySubjects.map((s) => s.id);
    const dateRecords = attendanceRecords.filter(
      (r) => r.date === selectedDate && facultySubjectIds.includes(r.subjectId)
    );
  ...
  }, [selectedDate, facultySubjects]);

  // Subject-wise Report Data
  const subjectWiseData = useMemo(() => {
    const filteredSubjects = selectedSubject === "all"
      ? facultySubjects
      : facultySubjects.filter((s) => s.id === selectedSubject);
  ...
  }, [selectedSubject, dateRange, attendanceThreshold, facultySubjects]);

  // Semester Report Data - Overall student attendance across faculty's subjects
  const semesterData = useMemo(() => {
    const facultySubjectIds = facultySubjects.map((s) => s.id);

    return students
      .map((student) => {
        // Calculate attendance only for faculty's subjects
        const relevantRecords = attendanceRecords.filter(
          (r) => r.studentId === student.id && facultySubjectIds.includes(r.subjectId)
        );

        const total = relevantRecords.length;
        const present = relevantRecords.filter((r) => r.status === "present").length;
        const percentage = total > 0 ? Math.round((present / total) * 100) : 0;
        const eligible = percentage >= attendanceThreshold;

        // Get subject-wise breakdown
        const subjectWise = facultySubjects.map((subject) => {
          const subjRecords = relevantRecords.filter((r) => r.subjectId === subject.id);
          const subjPresent = subjRecords.filter((r) => r.status === "present").length;
          const subjTotal = subjRecords.length;
          return {
            subject,
            present: subjPresent,
            total: subjTotal,
            percentage: subjTotal > 0 ? Math.round((subjPresent / subjTotal) * 100) : 0,
          };
        });
  ...
  }, [attendanceThreshold, searchQuery, showOnlyDefaulters, facultySubjects]);
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              View attendance for your assigned subjects
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => exportToCSV(semesterData, "faculty-attendance-report")}>
        ...
        <TabsContent value="day" className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              ...
              {dayWiseData.subjectStats.length > 0 && (
                <div className="mb-6">
                  ...
                  <tbody>
                    {dayWiseData.absentees.length > 0 ? (
                      dayWiseData.absentees.map(({ student, records }) => {
                        // Calculate attendance only for faculty subjects
                        const facultySubjectIds = facultySubjects.map((s) => s.id);
                        const relevantRecords = attendanceRecords.filter(
                          (r) => r.studentId === student.id && facultySubjectIds.includes(r.subjectId)
                        );
        ...
        <TabsContent value="subject" className="space-y-6">
          <Card>
            ...
            <CardContent>
              ...
              {selectedSubject !== "all" && subjectWiseData[0]?.studentAttendance.length > 0 && (
                <div>
        ...
        <TabsContent value="semester" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Final eligibility report for your subjects</CardDescription>
            </CardHeader>
            <CardContent>
              {/* Filters */}
              <div className="flex flex-wrap items-center gap-4 mb-6">
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-center py-3 px-4 font-medium">Status</th>
                      <th className="text-center py-3 px-4 font-medium">Weak Subjects</th>
                    </tr>
                  </thead>
                  <tbody>
                    {semesterData.length > 0 ? (
